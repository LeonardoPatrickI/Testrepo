...
class DataProcessor:
    ...
    def balance_data(self, df):
        try:
            ...
            logger.info(f"Error during balancing data step {e}")
    ...
    def select_features(self, df):
        try:
            ...
            topfeature_importance_df = feature_importance_df.sort_values(by='importance', ascending=False)
            num_features_to_select = self.config["data_processing"]["no_of_features"]
            top_10_features = topfeature_importance_df["feature"].head(num_features_to_select)
            logger.info(f"Features selected : {top_10_features}")
            top_10_df = df[top_10_features.to_list() + ['booking_status']]
    ...
    def save_data(self, df, file_path):
        try:
            ...
            logger.info(f"Data saved successfuly to {file_path}]")
        except Exception as e:
    ...
    def process(self):
        try:
            ...
            logger.info(f"Error during preprocessing pipeline step {e}")
